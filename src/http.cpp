                           ...
                           std::optional<std::int64_t> abuse_cache_rows)
{
    ...
    out += ",\"abuse_enabled\":";
    out += abuse_enabled ? "true" : "false";
    out += ",\"intel_enabled\":";
    out += intel_enabled ? "true" : "false";
    out += ",\"home_configured\":";
    out += home_configured ? "true" : "false";
    out += ",\"home_valid\":";
    out += home_valid ? "true" : "false";
    out += ",\"intel_last_refresh_ts\":";
    append_i64_or_null(out, intel_last_refresh_ts);
    out += ",\"abuse_cache_rows\":";
    append_i64_or_null(out, abuse_cache_rows);
    out += ",\"db_size_bytes\":";
                           ...
                           [[maybe_unused]] void**       con_cls)
{
    ...
    if (url_sv == "/api/status") {
        const auto snapshot = ctx->db->status_snapshot();
        if (!snapshot.has_value()) {
            return send_response(conn, MHD_HTTP_SERVICE_UNAVAILABLE,
                                 "application/json", "{\"ok\":false}",
                                 "no-store");
        }
        const bool home_configured = ctx->home_resolver != nullptr;
        const bool home_valid = home_configured && ctx->home_resolver->get().valid;
        const std::string body = status_to_json(
            *snapshot,
            ctx->abuse_enabled,
            ctx->intel_enabled,
            home_configured,
            home_valid,
            ctx->intel_cache != nullptr ? ctx->intel_cache->last_refresh_ts() : std::nullopt,
            ctx->abuse_cache != nullptr ? ctx->abuse_cache->cache_row_count() : std::nullopt);
        return send_response(conn,
                             snapshot->ok ? MHD_HTTP_OK : MHD_HTTP_SERVICE_UNAVAILABLE,
                             "application/json", body, "no-store");
    }
...
HttpServer::HttpServer(std::uint16_t       port,
    ...
    : db_(db),
      ctx_{&db_, home_resolver, abuse_cache, intel_cache, abuse_enabled, intel_enabled}
{
